"""
Q-MAML Algorithm 1 (pre-training) / Algorithm 2 (adaptation) applied to the D8 lattice gauge theory
(hamiltonians_d8lgt.py). Mirrors scalar_qmaml.py / susyqm_qmaml.py's structure (H is a dense Hermitian
matrix over a mixed fermion+link Hilbert space, not a Pauli sum -- measured via qml.Hermitian,
...
from hamiltonians_d8lgt import d8_lgt_hamiltonian, param_shape, NUM_QUBITS
...
class D8LGTPQC(nn.Module):
    """Hardware-efficient (StronglyEntanglingLayers) ansatz PQC whose cost is
    <psi(theta)|H_D8LGT(M,lambda_E,epsilon)|psi(theta)>, measured via qml.Hermitian."""
...
def pretrain_learner_d8lgt(num_qubits, depth, train_tasks, epochs, lr, w0_scale=0.3, log_every=5):
    """Algorithm 1: no inner loop -- backprop straight through Learner(M,lambda_E,epsilon) -> PQC cost."""
    shape = param_shape(depth, num_qubits)
    learner = Learner(3, shape, hidden=256).double()  # task descriptor = (M, lambda_E, epsilon)
    pqc = D8LGTPQC(num_qubits, depth)
    opt = torch.optim.Adam(learner.parameters(), lr=lr)

    loss_hist, grad_hist = [], []
    for epoch in range(epochs):
        epoch_loss, epoch_grad = 0.0, []
        for M, lam, eps in train_tasks:
            H = d8_lgt_hamiltonian(M, lam, eps)
            opt.zero_grad()
            task_vec = torch.tensor([M, lam, eps], dtype=torch.float64)
        ...
        if epoch % log_every == 0 or epoch == epochs - 1:
            print(f"[D8LGT pretrain] epoch {epoch+1}/{epochs}  meanE={loss_hist[-1]:.4f}  gradnorm={grad_hist[-1]:.4f}")
    return learner, pqc, {"loss": loss_hist, "grad_norm": grad_hist}


def adapt_task_d8lgt(pqc, theta0, H, iters, lr):
...
__all__ = ["classical_init", "D8LGTPQC", "pretrain_learner_d8lgt", "adapt_task_d8lgt", "NUM_QUBITS"]
