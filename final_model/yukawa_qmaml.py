"""
Q-MAML Algorithm 1 (pre-training) / Algorithm 2 (adaptation) applied to the scalar Yukawa coupling
Hamiltonian (hamiltonians_yukawa.py). Mirrors scalar_qmaml.py / susyqm_qmaml.py's structure (H is a dense
Hermitian matrix, not a Pauli sum; default.qubit + backprop).
"""
import numpy as np
import torch
...
from hamiltonians_yukawa import yukawa_hamiltonian, param_shape, NUM_QUBITS
...
class YukawaPQC(nn.Module):
    """Hardware-efficient (StronglyEntanglingLayers) ansatz PQC whose cost is
    <psi(theta)|H_Yukawa(eta,m)|psi(theta)>, measured via qml.Hermitian."""
...
def pretrain_learner_yukawa(num_qubits, depth, train_tasks, epochs, lr, w0_scale=0.3, log_every=5):
    """Algorithm 1: no inner loop -- backprop straight through Learner(eta,m) -> PQC cost."""
    shape = param_shape(depth, num_qubits)
    learner = Learner(2, shape, hidden=256).double()  # task descriptor = (eta, m)
    pqc = YukawaPQC(num_qubits, depth)
    opt = torch.optim.Adam(learner.parameters(), lr=lr)

    loss_hist, grad_hist = [], []
    for epoch in range(epochs):
        epoch_loss, epoch_grad = 0.0, []
        for eta, m in train_tasks:
            H = yukawa_hamiltonian(eta, m)
            opt.zero_grad()
            task_vec = torch.tensor([eta, m], dtype=torch.float64)
        ...
        if epoch % log_every == 0 or epoch == epochs - 1:
            print(f"[Yukawa pretrain] epoch {epoch+1}/{epochs}  meanE={loss_hist[-1]:.4f}  gradnorm={grad_hist[-1]:.4f}")
    return learner, pqc, {"loss": loss_hist, "grad_norm": grad_hist}


def adapt_task_yukawa(pqc, theta0, H, iters, lr):
...
__all__ = ["classical_init", "YukawaPQC", "pretrain_learner_yukawa", "adapt_task_yukawa", "NUM_QUBITS"]
