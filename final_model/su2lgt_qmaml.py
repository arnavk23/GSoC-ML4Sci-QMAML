"""
Q-MAML Algorithm 1 (pre-training) / Algorithm 2 (adaptation) applied to the SU(2) lattice gauge theory
(hamiltonians_su2lgt.py). Mirrors schwinger_qmaml.py / z2lgt_qmaml.py's structure: task vector (m_tilde, x),
lightning.qubit + adjoint differentiation (a genuine Pauli-sum Hamiltonian, same device class as Tracks
A/B). MVP scope: N=2 (4 qubits), the un-reduced Hamiltonian -- see hamiltonians_su2lgt.py's docstring for
what this does and does not reproduce from the source paper.
"""
import numpy as np
import torch
...
from hamiltonians_su2lgt import su2_lgt_hamiltonian, param_shape
...
class SU2LGTPQC(nn.Module):
    """Hardware-efficient (StronglyEntanglingLayers) ansatz PQC whose cost is
    <psi(theta)|H_SU2LGT(m_tilde,x)|psi(theta)>, measured directly via qml.expval(H)."""
...
def pretrain_learner_su2lgt(num_qubits, depth, train_tasks, epochs, lr, w0_scale=0.3, log_every=5):
    """Algorithm 1: no inner loop -- backprop straight through Learner(m_tilde,x) -> PQC cost."""
    shape = param_shape(depth, num_qubits)
    N = num_qubits // 2
    learner = Learner(2, shape, hidden=256).double()  # task descriptor = (m_tilde, x)
    pqc = SU2LGTPQC(num_qubits, depth)
    opt = torch.optim.Adam(learner.parameters(), lr=lr)

    loss_hist, grad_hist = [], []
    for epoch in range(epochs):
        epoch_loss, epoch_grad = 0.0, []
        for m_tilde, x in train_tasks:
            H = su2_lgt_hamiltonian(N, m_tilde, x)
            opt.zero_grad()
            task_vec = torch.tensor([m_tilde, x], dtype=torch.float64)
        ...
        if epoch % log_every == 0 or epoch == epochs - 1:
            print(f"[SU2LGT pretrain] epoch {epoch+1}/{epochs}  meanE={loss_hist[-1]:.4f}  gradnorm={grad_hist[-1]:.4f}")
    return learner, pqc, {"loss": loss_hist, "grad_norm": grad_hist}


def adapt_task_su2lgt(pqc, theta0, H, iters, lr):
...
__all__ = ["classical_init", "SU2LGTPQC", "pretrain_learner_su2lgt", "adapt_task_su2lgt", "NUM_QUBITS"]
